# QuadPlan — VPS Installation Guide

Step-by-step guide for installing QuadPlan on a remote VPS (Hetzner/Ubuntu). Based on real deployment notes. Designed for both humans and AI coding agents.
...
**Why CPX32:** QuadPlan runs 4 concurrent AI agents, each spawning its own PTY + subprocess. 4 vCPUs map to the 4-agent model, 8 GB RAM provides headroom. Smaller plans may work for testing.
...
Claude Code blocks `--dangerously-skip-permissions` when running as root. Agents will crash immediately if QuadPlan runs under root.
...
mkdir -p /projects
chown quadplan:quadplan /projects
```
...
```bash
mkdir -p /home/quadplan/.ssh
cp /root/.ssh/authorized_keys /home/quadplan/.ssh/authorized_keys
chown -R quadplan:quadplan /home/quadplan/.ssh
chmod 700 /home/quadplan/.ssh
chmod 600 /home/quadplan/.ssh/authorized_keys
```

Update local `~/.ssh/config` — change `User root` to `User quadplan`. **All subsequent steps run as the `quadplan` user.**
...
**Do NOT use system Node (`apt` or `nodesource`).** Only use nvm. System Node alongside nvm creates PATH conflicts — pm2 and QuadPlan spawn agents with system PATH (missing nvm binaries), causing agents to fail auth or not be found.
...
All binaries (`claude`, `codex`, `quadplan`, `pm2`) live under `~/.nvm/versions/node/v24.x.x/bin/`.

### Authenticate CLIs

> **These are interactive steps.** The operator must run these via `ssh quadplan`:
...
ssh quadplan-old 'tar czf /tmp/auth-backup.tar.gz .claude .codex .config/gh'
scp quadplan-old:/tmp/auth-backup.tar.gz /tmp/
scp /tmp/auth-backup.tar.gz quadplan:/tmp/
ssh quadplan 'cd ~ && tar xzf /tmp/auth-backup.tar.gz && rm /tmp/auth-backup.tar.gz'
```

---

## Step 7: Install QuadPlan
...
**IMPORTANT:** pm2 strips PATH from child processes. Even if nvm is loaded, the QuadPlan process won't have nvm binaries in PATH. **Do not fix with symlinks** — they resolve the binary but not the environment.
...
# sudo env PATH=... pm2 startup systemd -u quadplan --hp /home/quadplan
```
...
pm2 logs quadplan                 # Live logs
pm2 logs quadplan --lines 50 --nostream  # Last 50 lines
pm2 stop quadplan                 # Stop
pm2 start quadplan                # Start
pm2 restart quadplan              # Restart
...
**How it works:**
- First visit: no `qw_auth` cookie → `$auth_ok` = `"QuadPlan"` → browser prompts for credentials (normal basic auth)
...
2. SSH in as root, create `quadplan` user with sudo + SSH keys
3. Update local SSH config to `User quadplan`
...
9. Install QuadPlan + pm2
10. Run `quadplan init`
11. Create `~/start-quadplan.sh` wrapper script (loads nvm before exec)
